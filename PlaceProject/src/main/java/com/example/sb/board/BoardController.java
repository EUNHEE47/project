...
//      System.out.println("endBlock : " + endBlock);
	...
	public String boardCreate(@Valid BoardForm boardForm, BindingResult bindingResult,
			Principal principal/*
								 * , @RequestParam Integer pcode1, @RequestParam(defaultValue = "0") Integer
								 * pcode2, @RequestParam(defaultValue = "0") Integer pcode3
								 */) {
		...
		boardService.create(boardForm.getTitle(), boardForm.getContent(), userEntity, boardForm.getPcode1(),
				boardForm.getPcode2(), boardForm.getPcode3());
	...
	public String detail(Model model, @PathVariable("bCode") Long bCode, CommForm commForm) {
		 ...
		 * SearchPlace pcode2 = null; if(be.getPCode2() != null) pcode2 =
		 * searchPlaceRepository.findBypCode(be.getPCode2()); SearchPlace pcode3 = null;
		 * if(be.getPCode3() != null) pcode3 =
		 * searchPlaceRepository.findBypCode(be.getPCode3());
		 * 
		 * 
		 * List<SearchPlace> pcode = new ArrayList<>();
		 * 
		 * pcode.add(pcode1); if(be.getPCode2() != null) pcode.add(pcode2);
		 * if(be.getPCode3() != null) pcode.add(pcode3);
		 * 
	...
	/*
	 * modify_form 에서 수정하기 버튼 누르면 관련 타이틀, 콘텐트 값 DB에 update시키기
	 */
	...
	public String boardModify(@Valid BoardForm boardForm, BindingResult bindingResult,
		...
		boardService.boardModify(bCode, boardForm.getTitle(), boardForm.getContent(), boardForm.getPcode1(),
				boardForm.getPcode2(), boardForm.getPcode3());
